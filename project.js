const projects = [
    {
        title: "Ingredient Brand A/B Testing for Cost Optimization (Multi-Outlet F&B)",
        description: "Ran a 4-outlet cross-over A/B test to identify the most cost-effective ingredient brand while preserving product taste. Result: ~15% reduction in ingredient cost per product with no increase in customer complaints.",
        tech: ["A/B Testing", "Data Analysis", "Cost Optimization", "F&B Operations"],
        link: "#",
        type: "private",
        category: "data"
    },
    {
        title: "Ingredient Reorder Point Optimization (Inventory Planning – F&B)",
        description: "Built a data-driven reorder point model using a 10-week weighted moving average to trigger proactive purchasing. Result: Reduced emergency orders, estimated 5–10% ingredient cost savings, and improved cashflow and warehouse utilization.",
        tech: ["Inventory Planning", "Weighted Moving Average", "Data Analysis", "F&B Operations"],
        link: "#",
        type: "private",
        category: "data"
    },
    {
        title: "Sales Analytics & Automation Platform (F&B)",
        description: "Production-ready analytics platform supporting 7 retail outlets. Built automated Python-BigQuery pipelines and executive dashboards. Private due to NDA.",
        tech: ["Python", "BigQuery", "Power BI", "Looker Studio", "PyInstaller"],
        link: "#",
        type: "private",
        category: "data"
    },
    {
        title: "Inventory System",
        description: "Internal system to manage ingredients, products, and worker access. Private published due to NDA",
        tech: ["Next.js", "Tailwind CSS", "PostgreSQL", "ChartJS"],
        link: "#",
        type: "private",
        category: "dev"
    },
    {
        title: "Content Management System (R&D Ingredient Library)",
        description: "Built an internal CMS for Adabi's R&D team to centralize ingredient, premix, and product data. The system supports detailed records, searchable listings, and role-based access, improving data accessibility and reducing manual lookups.",
        tech: ["Next.js", "Tailwind CSS", "Node.js", "REST APIs", "MySQL"],
        link: "#",
        type: "private",
        category: "dev"
    },
    {
        title: "Malaysian Translator Associate",
        description: "Architected a high-performance localization platform using a Headless CMS and Next.js. Engineered custom content models for multi-language support and optimized editorial workflows for the Malaysian market.",
        tech: ["Next.js", "Headless CMS", "Tailwind CSS", "SEO"],
        link: "https://mtappm-frontend.vercel.app/",
        type: "public",
        category: "dev"
    },
    {
        title: "IOTEG",
        description: "A professional engineering solution homepage and headless e-commerce store. Currently developing innovative IoT devices aimed at reducing motorcycle vibration and enhancing the riding experience.",
        tech: ["Next.js", "WooCommerce", "Headless E-commerce", "Tailwind CSS"],
        link: "https://ioteg.com/",
        type: "public",
        category: "dev"
    },
    {
        title: "Magic Chess Draft Tool",
        description: "A specialized utility designed to facilitate 4v4 team scrims by providing a drafting system currently missing from the official Magic Chess Go Go application.",
        tech: ["React + Vite", "Tailwind CSS", "Firebase"],
        link: "https://magic-chess-draft.vercel.app/",
        type: "public",
        category: "dev"
    }
];
window.projects = projects;